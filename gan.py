...
from transformer import Transformer, Encoder, _input_tf
...
def _dirty_real_input(time, instrument, note, state, num_ins=20):
    time = time * tf.random.normal(shape=tf.shape(time), mean=10, stddev=0.5)
...
def _train_step(time, instrument, note, state, gen, dis, gen_opt, dis_opt, max_ins=129):
    ...
    def upd():
        ...
        return 1
    balance = tf.cond(loss1 > loss4, lambda: 0, upd)
    del tape
    return loss1, loss2, loss3, balance
...
def train():
    ...
    Train the model
    """
    data, ins = _input_tf(sequence=SEQUENCE_LENGTH, batch=BATCH_SIZE)
    vec_size = 3 + ins
    gen = Generator(output_size=vec_size)
    dis = Discriminator()
    gen_opt = tf.keras.optimizers.Adam(LEARNING_RATE_GEN, 0.9, 0.98, 1e-9)
    dis_opt = tf.keras.optimizers.Adam(LEARNING_RATE_DIS, 0.9, 0.98, 1e-9)
